...
#Get player ID
playerDlg = gui.Dlg(title = "21 Practice Game")
...
#Create window
win = visual.Window([800,600], monitor = 'testMonitor')#, fullscr = useFullScreen)
#Instructions Screen
...
for i in range(1,11):
    ...
    if sum(players_cards) != 21 and sum(computers_cards) == 21:
        ...
        win.flip()
    elif sum(players_cards) > 21 and sum(computers_cards) < 21:
        winning = 0
        ...
        if sum(players_cards) == sum(computers_cards):
            winning = 1
            computer_draw_player_text = ' You and the computer tied.It\'s a draw. \nThese are the computer\'s cards: ' + str(computers_cards)
            computer_draw_player_screen = visual.TextStim(win, font = 'Arial', text = computer_beat_player_text)
            wallet = wallet
            computer_draw_player_screen.draw()
            win.flip()
        #Wins
    ...
    #Draw
        ...
        win.flip()
    if sum(players_cards) == 21 or sum(computers_cards) == 21 or sum(players_cards) > 21 or sum(computers_cards) > 21:
        difference = 'NA'
    else:
        difference = sum(players_cards) - sum(dealers_cards)
    core.wait(5.0)
    n = 0
    conf_level = 0
    excited = 0
    ...
    distressed = 0
    ashamed = 0
    data_to_write = str(i) + ',' + str(n) + ',' + str(bet)+ ',' + str(wallet) + ',' + str(winning) + ',' + str(len(players_cards)-2) + ',' + str(difference) + ',' + str(conf_level) + ',' + str(excited) + ',' + str(proud) + ',' + str(strong) + ',' + str(nervous) + ',' + str(distressed)+ ',' + str(ashamed) + '\n'
